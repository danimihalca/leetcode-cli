...
plugin.getStudyPlan = function(argv, cb) {
  log.debug('running leetcode.getStudyPlan');
  ...
  opts.body = {
    ...
    variables:     {slug: argv.plan},
    operationName: 'studyPlanDetail'
  };

  const spin = h.spin('Downloading ' + argv.plan);
  request.post(opts, function(e, resp, body) {
    spin.stop();
    e = plugin.checkError(e, resp, 200);
    if (e) return cb(e);

    const content = body.data.studyPlanV2Detail;
    if (!content) return cb('failed to load study plan!');
